 ...
 */
export function formatNum (num: number | string, char: string, pre: string = '') {
  if (
 ...
 */
export function toFixed (num: string | number, len: number, type = '') {
  ...
  if (_len <= len) {
    ...
    let res, nextBit
    const initNum = parseInt(numList[0] + '' + decimal.substr(0, len) + '.' + decimal.substr(len), 10)
    switch (type) {
      case 'ceil':
 ...
 */
export function scm (num1: number, num2: number) {
